...
//            for (int j=0; j<5; j++) {
//                sprintf(msg,"k: %d j: %d x: %f y: %f",k,j,x[j],y[j]);
//                LOG_MSG(msg);
//            }
            if (fixed_yscale == 0) {
                double ylast = y[n-1];
//                LOG_QMSG("not fixed_yscale");
//                if (ylast > yscale) {
//                    yscale = max(yscale,calc_yscale_ts(ylast));
//                }
                yscale = max(yscale,ylast);
//                sprintf(msg,"n: %d ylast: %f yscale: %f",n,ylast,yscale);
//                LOG_MSG(msg);
            } else {
                LOG_QMSG("fixed_yscale");
                yscale = fixed_yscale;
            }
